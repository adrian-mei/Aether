        ...
        <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-4 md:gap-0">
          ...
          <div className="flex items-center gap-2 self-end md:self-auto">
            <div className={`w-2 h-2 rounded-full ${
              uiVoiceState === 'error' ? 'bg-rose-500' :
              uiVoiceState !== 'idle' ? 'bg-emerald-400 animate-pulse' : 'bg-emerald-600/50'
            }`} />
            <span className={`text-xs uppercase tracking-wider ${
              uiVoiceState === 'error' ? 'text-rose-400/80' : 'text-emerald-400/50'
            }`}>
              {uiVoiceState === 'error' ? 'Offline' :
